...
[name]='ShellBot'
[keywords]='API de Shell Script Telegram'
[description]='API no oficial para crear bots en la plataforma Telegram.'
[version]='6.0'
...
# Erros
readonly _ERR_TYPE_BOOL_='tipo incompatible: solo admite "true" o "false".'
readonly _ERR_TYPE_INT_='tipo incompatible: solo admite inteiro.'
readonly _ERR_TYPE_FLOAT_='tipo incompatible: solo admite float.'
readonly _ERR_PARAM_REQUIRED_='opción requerida: compruebe si o(s) parámetro(s) o argumento(s) obligatorio(s) están presentes(s).'
readonly _ERR_TOKEN_UNAUTHORIZED_='No autorizado: compruebe si tiene permisos para usar el token.'
readonly _ERR_TOKEN_INVALID_='token no válido: verifique el número de token e intente nuevamente.'
readonly _ERR_BOT_ALREADY_INIT_='acción no permitida: el bot ya se ha inicializado.'
readonly _ERR_FILE_NOT_FOUND_='acceso fallido: no se puede leer el archivo.'
readonly _ERR_DIR_WRITE_DENIED_='permiso denegado: no se puede escribir en el directorio.'
readonly _ERR_DIR_NOT_FOUND_='No se puede acceder: directorio no encontrado.'
readonly _ERR_FILE_INVALID_ID_='ID inválido: archivo no encontrado.'
readonly _ERR_UNKNOWN_='error desconocido: se produjo un error inesperado. Informar el problema al desarrollador.'
...
readonly _ERR_SERVICE_SYSTEMD_NOT_FOUND_='error al activar: el sistema no admite la gestión de servicios "systemd".'
readonly _ERR_SERVICE_USER_NOT_FOUND_='Usuario no encontrado: la cuenta de usuario ingresada no es válida.'
readonly _ERR_VAR_NAME_='variable no encontrada: el identificador no es válido o no existe.'
readonly _ERR_FUNCTION_NOT_FOUND_='función no encontrada: el identificador especificado no es válido o no existe.'
readonly _ERR_ARG_='argumento no válido: argumento no admitido por el parámetro especificado.'
readonly _ERR_RULE_ALREADY_EXISTS_='Error al establecer: el nombre de la regla ya existe.'
readonly _ERR_HANDLE_EXISTS_='registro de error: ya hay un identificador vinculado a la devolución de llamada'
readonly _ERR_CONNECTION_='falla de conexión: no se puede establecer conexión con Telegram.'
...
CreateUnitService()
{
	...
	local fail='\033[0;31m[FALLA]\033[0;m'
